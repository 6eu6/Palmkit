    ...
    const components = useMemo(() => {
      return {
        div: ({ className, children, node, ...props }) => {
          ...
          if (className?.includes('__palmkitArtifact__')) {
          ...
          if (className?.includes('__palmkitSelectedElement__')) {
            const messageId = node?.properties.dataMessageId as string;
            ...
            return (
              <div className="bg-palmkit-elements-background-depth-3 border border-palmkit-elements-borderColor rounded-lg p-3 my-2">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs font-mono bg-palmkit-elements-background-depth-2 px-2 py-1 rounded text-palmkit-elements-textTer">
                    {elementData?.tagName}
                  </span>
                  {elementData?.className && (
                    <span className="text-xs text-palmkit-elements-textSecondary">.{elementData.className}</span>
                  )}
                </div>
                <code className="block text-sm !text-palmkit-elements-textSecondary !bg-palmkit-elements-background-depth-2 border border-palmkit-elements-borderColor p-2 rounded">
          ...
          if (className?.includes('__palmkitThought__')) {
        ...
        pre: (props) => {
          ...
          if (
            ...
            /*
             * Spread FIRST so the explicit props below always win — `rest`
             * comes from hast element properties whose loosely typed
             * `language` key (string) failed typecheck when it could
             * override the strictly-typed prop.
             */
            return (
              <CodeBlock
                {...rest}
        ...
        button: ({ node, children, ...props }) => {
          ...
          if (
            dataProps?.class?.toString().includes('__palmkitQuickAction__') ||
            dataProps?.dataPalmkitQuickAction === 'true'
          ) {
            ...
            return (
              <button
                className="rounded-md justify-center px-3 py-1.5 text-xs bg-palmkit-elements-item-backgroundAccent text-palmkit-elements-item-contentAccent opacity-90 hover:opacity-100 flex items-center gap-2 cursor-pointer"
                data-type={type}
 ...
 * const input = "```xml\n<div class='__palmkitArtifact__'></div>\n```";
 * stripCodeFenceFromArtifact(input);
 * // Returns: "\n<div class='__palmkitArtifact__'></div>\n"
 *
 * @remarks
 * - Only removes code fences that directly wrap an artifact (marked with __palmkitArtifact__ class)
 * - Handles code fences with optional language specifications (e.g. ```xml, ```typescript)
 * - Preserves original content if no artifact is found
 * - Safely handles edge cases like empty input or artifacts at start/end of content
 */
export const stripCodeFenceFromArtifact = (content: string) => {
  if (!content || !content.includes('__palmkitArtifact__')) {
    return content;
  }

  const lines = content.split('\n');
  const artifactLineIndex = lines.findIndex((line) => line.includes('__palmkitArtifact__'));
