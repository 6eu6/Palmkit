 ...
 * ReadFileRenderer
 * ================
 * Renders the result of the `read_file` tool.
 *
 * Shows: file path, size, line count, content (with line numbers).
 * Code files get syntax-highlighted via the existing CodeBlock component.
 */
...
function ReadFileRendererImpl({ result, theme: _theme }: ReadFileRendererProps) {
  const [showLineNumbers, setShowLineNumbers] = useState(true);

  const data = result as ReadFileResult;
        ...
        meta={
          <>
            ...
            {data.lines !== undefined && (
              <span className="text-xs text-palmkit-elements-textSecondary">{data.lines} lines</span>
            )}
          </>
        }
      />
...
export const ReadFileRenderer = memo(ReadFileRendererImpl);
