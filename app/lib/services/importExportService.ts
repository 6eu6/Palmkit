...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          palmkit_user_profile: this._safeGetItem('palmkit_user_profile'),
          palmkit_settings: this._safeGetItem('palmkit_settings'),
          palmkit_profile: this._safeGetItem('palmkit_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('palmkit_viewed_features'),
          developer_mode: this._safeGetItem('palmkit_developer_mode'),
        ...
        ui: {
          // Tab configuration
          palmkit_tab_configuration: this._safeGetItem('palmkit_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('palmkit_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('palmkit_acknowledged_connection_issue'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('palmkit_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('palmkit_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.palmkit_tab_configuration) {
        try {
          this._safeSetItem('palmkit_tab_configuration', data.ui.palmkit_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'palmkit_acknowledged_debug_issues',
        'palmkit_acknowledged_connection_issue',
        'error_logs',
        'palmkit_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('palmkit_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
