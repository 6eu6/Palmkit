...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  ...
  private _hasDetectedArtifacts(input: string): boolean {
    return input.includes('<palmkitArtifact') || input.includes('</palmkitArtifact>');
  }
  ...
  private _wrapInArtifact(artifactId: string, filePath: string, content: string): string {
    ...
    return `<palmkitArtifact id="${artifactId}" title="${title}" type="bundled">
<palmkitAction type="file" filePath="${filePath}">
${content}
</palmkitAction>
</palmkitArtifact>`;
  }
  ...
  private _wrapInShellAction(content: string, messageId: string): string {
    ...
    return `<palmkitArtifact id="${artifactId}" title="Shell Command" type="shell">
