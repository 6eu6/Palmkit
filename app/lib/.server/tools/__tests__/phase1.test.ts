...
await test('scrape_page schema rejects invalid extract field', () => {
  const result = scrapePageSchema.safeParse({
    url: 'https://example.com',
...
await test('SCRAPE_PAGE_DEFAULT_FIELDS has 6 fields', () => {
...
await test('resolveToolMode: build -> code', () => {
...
await test('resolveToolMode: discuss + work -> work', () => {
  assert.equal(resolveToolMode('discuss', 'work'), 'work');
});

await test('resolveToolMode: discuss + chat -> chat', () => {
  assert.equal(resolveToolMode('discuss', 'chat'), 'chat');
});

await test('resolveToolMode: discuss + undefined -> chat', () => {
  assert.equal(resolveToolMode('discuss', undefined), 'chat');
});

await test('resolveToolMode: discuss + code -> code', () => {
  assert.equal(resolveToolMode('discuss', 'code'), 'code');
});

await test('web_search available in all 3 modes', () => {
...
await test('read_url available in all 3 modes', () => {
...
await test('scrape_page available in work mode only', () => {
...
await test('registry.listToolsForMode(chat) returns lightweight set', () => {
...
await test('registry.listToolsForMode(work) returns expanded set', () => {
...
await test('registry.listToolsForMode(code) returns dev toolset', () => {
...
await test('registry.listAllTools returns expected count', () => {
  const all = toolRegistry.listAllTools();

  /*
   * Phase 1: 3 investigative native + 6 code-mode legacy + 6 work-mode legacy = 15
   * Phase 2: + 4 office native + 2 analytics native - 2 replaced legacy (create_document, format_table) = 19
   */
  assert.equal(all.length, 19, `Expected 19 tools, got ${all.length}`);
});

await test('registry.getToolByName returns definition', () => {
  const t = toolRegistry.getToolByName('web_search');
  assert.ok(t);
  assert.equal(t!.name, 'web_search');
});

await test('registry.getToolByName returns undefined for unknown', () => {
  const t = toolRegistry.getToolByName('nonexistent_tool');
  assert.equal(t, undefined);
});

await test('registry.getToolsForMode returns AI SDK tool objects with execute', () => {
...
await test('registry.getToolsForMode binds ctx via closure', async () => {
...
await test('web_search returns error when no API key', async () => {
  const ctx = makeCtx({ mode: 'chat', searchApiKey: undefined });
  const result = await webSearchTool.execute({ query: 'test' }, ctx);

  assert.equal(result.ok, false);
  assert.ok((result as any).error.includes('not configured'));
});

await test('web_search returns results from Tavily', async () => {
  mockFetch({
...
await test('read_url returns text content from HTML', async () => {
  mockFetch({
    'example.com': {
...
await test('read_url returns JSON for JSON endpoints', async () => {
  mockFetch({
    'api.example.com': {
...
await test('read_url returns error for 404', async () => {
  mockFetch({
...
await test('read_url supports format=markdown', async () => {
  mockFetch({
    'example.com': {
      headers: { 'content-type': 'text/html' },
...
await test('scrape_page returns structured data', async () => {
  const html = `
    <html>
    <head>
...
await test('scrape_page respects extract filter', async () => {
...
await test('scrape_page extracts tables', async () => {
  const html = `
    <html><body>
    <table>
...
await test('scrape_page extracts code blocks with language', async () => {
  const html = `
    <html><body>
...
await test('scrape_page returns error for non-HTML', async () => {
  mockFetch({
    'api.example.com': {
      headers: { 'content-type': 'application/json' },
      body: { hello: 'world' },
...
await test('web_search description mentions search and web', () => {
...
await test('web_search description warns against using it for URLs', () => {
  const desc = webSearchTool.description.toLowerCase();
  assert.ok(desc.includes('url') || desc.includes('do not'), 'should mention URL anti-pattern');
});

await test('read_url description mentions fetch and content', () => {
...
await test('read_url description distinguishes from scrape_page', () => {
  const desc = readUrlTool.description.toLowerCase();
  assert.ok(desc.includes('scrape_page'), 'should mention scrape_page as alternative');
});

await test('scrape_page description mentions analyze and structured', () => {
...
await test('scrape_page description restricts to work mode', () => {
  const desc = scrapePageTool.description.toLowerCase();
  assert.ok(desc.includes('work mode'), 'should mention work mode');
});

await test('every tool has a non-trivial description (>100 chars)', () => {
  const all = toolRegistry.listAllTools();
...
await test('every tool name is snake_case', () => {
  const all = toolRegistry.listAllTools();
...
await test('every tool name is unique', () => {
  const all = toolRegistry.listAllTools();
...
await test('every tool has at least one mode in availableIn', () => {
  const all = toolRegistry.listAllTools();
...
const MODE_EXPECTATIONS: Record<ToolMode, { has: string[]; notHas: string[] }> = {
  chat: {
    has: ['web_search', 'read_url'],
    notHas: [
      'scrape_page',
      'read_file',
      'list_files',
  ...
  code: {
    has: ['web_search', 'read_url', 'read_file', 'list_files', 'grep', 'run_shell', 'screenshot', 'read_sandbox_file'],
    notHas: [
      'scrape_page',
      'generate_image',
      'create_document',
...
for (const mode of ['chat', 'work', 'code'] as ToolMode[]) {
  await test(`mode=${mode}: contains expected tools`, () => {
    const tools = toolRegistry.listToolsForMode(mode);
    const names = tools.map((t) => t.name);

    for (const expected of MODE_EXPECTATIONS[mode].has) {
      assert.ok(names.includes(expected), `${mode} should include ${expected}`);
    }
  });

  await test(`mode=${mode}: excludes wrong-mode tools`, () => {
    const tools = toolRegistry.listToolsForMode(mode);
    const names = tools.map((t) => t.name);

    for (const forbidden of MODE_EXPECTATIONS[mode].notHas) {
      assert.ok(!names.includes(forbidden), `${mode} should NOT include ${forbidden}`);
    }
  });
}
...
await test('read_url handles empty response body', async () => {
  mockFetch({
    'empty.example.com': {
      headers: { 'content-type': 'text/html' },
      body: '',
...
await test('scrape_page handles page with no main content', async () => {
  mockFetch({
    'empty.example.com': {
      headers: { 'content-type': 'text/html' },
...
await test('scrape_page handles relative URLs in links', async () => {
...
await test('scrape_page deduplicates links by URL', async () => {
...
await test('scrape_page skips javascript: and mailto: links', async () => {
  const html = `<html><body>
...
/*
 * Note: we don't call process.exit() here because vitest treats it as a failure.
 * The test runner reports failures above; CI can grep for "FAILED: 0" to verify.
 */
