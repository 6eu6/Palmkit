  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__palmkitThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    button: [
      ...
      'name',
      'value',
      ['className', '__palmkitArtifact__', '__palmkitThought__', '__palmkitQuickAction'],
    ],
  },
  strip: [],
};
...
const limitedMarkdownPlugin: Plugin = () => {
  return (tree, file) => {
    ...
    visit(tree, (node: AnyNode, index: AnyNode, parent: AnyNode) => {
      if (
        index == null ||
