  ...
  try {
    switch (body.op) {
      ...
      case 'start': {
         ...
         *
         * Also inject `server.hmr: false`. The preview iframe loads `/preview/`
         * through our same-origin HTTP reverse proxy (functions/preview/[[path]].ts),
         * which does NOT forward WebSocket connections. Vite's HMR client still
         * tries to open a WS to `/preview/` and fails — flooding the browser
         * console with "failed to connect to websocket" errors. HMR is useless
         * in the preview anyway (the AI regenerates files, the user doesn't
         * live-edit in the sandbox), so disabling it cleanly removes the noise
         * without affecting the rendered app.
         */
        const patchHosts =
          `if ls vite.config.* >/dev/null 2>&1; then ` +
          `sed -ri 's/defineConfig\\(\\{/defineConfig({server:{allowedHosts:true,host:true,hmr:false},/; ` +
          `s/export default \\{/export default {server:{allowedHosts:true,host:true,hmr:false},/' vite.config.* 2>/dev/null || true; ` +
          `else echo "export default { server: { allowedHosts: true, host: true, hmr: false } }" > vite.config.js; fi`;
