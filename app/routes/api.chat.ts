  ...
  try {
    ...
    const dataStream = createDataStream({
      async execute(dataStream) {
         ...
         * Replaces the old builtInTools/workModeTools/phase2Tools imports
         * which were deleted. The registry handles mode filtering and
         * context binding (files, sandboxId) automatically.
         */
        ...
        for (const [name, toolObj] of Object.entries(registryTools)) {
          // Skip file tools when there are no files yet (fresh build)
          if (!hasExistingFiles && (name === 'read_file' || name === 'list_files')) {
            continue;
          }

          // Skip sandbox tools when there's no sandbox yet
