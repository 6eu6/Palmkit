...
async function supabaseUserLoader({ context }: { request: Request; context: any }) {
  try {
    // Try to get Supabase token from various sources
    const supabaseToken =
      context?.cloudflare?.env?.VITE_SUPABASE_ACCESS_TOKEN || process.env.VITE_SUPABASE_ACCESS_TOKEN;
