    ...
    switch (body.op) {
      ...
      case 'start': {
        ...
        const patchHosts =
          ...
          `sed -ri 's/defineConfig\\(\\{/defineConfig({server:{allowedHosts:true,host:true},/; ` +
          `s/export default \\{/export default {server:{allowedHosts:true,host:true},/' vite.config.* 2>/dev/null || true; ` +
          `else echo "export default { server: { allowedHosts: true, host: true } }" > vite.config.js; fi`;
