...
async function netlifyUserLoader({ context }: { request: Request; context: any }) {
  try {
    // Try to get Netlify token from various sources
    const netlifyToken = context?.cloudflare?.env?.VITE_NETLIFY_ACCESS_TOKEN || process.env.VITE_NETLIFY_ACCESS_TOKEN;
